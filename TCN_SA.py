...
TCN_model =TemporalConvNet(num_inputs=input_size, num_channels=[32,64,1],
...
#%%
# torch.save(best_model, '8-EC-speed/model/TCN_SH_SA.pth')
# torch.save(best_model, '8-EC-speed/model/TCN_SH_SANV.pth')
# torch.save(best_model, '8-EC-speed/model/TCN_SZ_SA.pth')
torch.save(best_model, '8-EC-speed/model/TCN_SZ_SANV.pth')
# torch.save(best_model, '8-EC-speed/model/TCN_CN_SA.pth')
# torch.save(best_model, '8-EC-speed/model/TCN_CN_SANV.pth')
#%%
"predict"
testX=torch.tensor(test_data[:,:,1:]).transpose(1, 2).float()
testY=torch.tensor(test_data[:,:,0]).unsqueeze(2).transpose(1, 2).float()
...
for size in [32,64,128,192,256,300]:
    ...
    with torch.no_grad():
        ...
        for i, (x, y) in enumerate(dataloaders_test):
            ...
            all_simu.append(Norm_pred.squeeze())
            all_real.append(y.squeeze())
